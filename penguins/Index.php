...
$filter = isset($_GET["cat"]) ? $_GET["cat"] : false; // Категория содержания

$sort = isset($_GET["sort"]) ? $_GET["sort"] : false; // сортировка по дате

$search = isset($_GET["search"]) ? $_GET["search"] : false; // поиск по дате

$page =  isset($_GET['page']) ? $_GET['page'] : 1; //номер страницы для пагинации


$query_search = "SELECT * FROM  `news` WHERE `title` LIKE '%$search%'"; // запрос на поиск
...
$offset = $page * $paginate_count - $paginate_count; // offset
    ...
    <main>
            ...
            <div class="sort">
                <ul class="list-group list-group-horizontal mt-5 mb-3">
                    ...
                    <li class="list-group-item">
                        <a href="/?sort=ASC<?= ($param != "") ? '&' . $param : '' ?>"><img width="30" src="icons\asc-sort.png" alt=""></a>
                    </li>
                    <li class="list-group-item">
                        <a href="/?sort=DESC<?= ($param != "") ? '&' . $param : '' ?>"><img width="30" src="icons\desc-sort.png" alt=""></a>
                    </li>
                </ul>
            </div>
        </section>
        <section class="last-news">
            <div class="container1">
                ...
                if ( mysqli_num_rows($count_news) == 0) {
                    echo "Данных нет";
